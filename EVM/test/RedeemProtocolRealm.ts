...
describe("RedeemProtocolRealm", function () {
    ...
    async function deployRealmWithMultiTokensTransfer() {
        ...
        const erc721A = await erc721Factory.deploy();
        const erc721B = await erc721Factory.deploy();
        const FF = await ethers.getContractFactory("PassportForwarder");
    ...
    describe("redeem", function () {
        ...
        it("should be reverted with second redemption", async function () {
            ...
            await expect(realm.connect(otherAccount).redeemWithMark(
                ...
                erc721A.address, 0, 0, otherAccount.address, ethers.utils.formatBytes32String(''),
            );
        ...
        it("should be success with empty custom id", async function () {
            ...
            await expect(realm.connect(otherAccount).redeemWithTransfer(
                ...
                erc721A.address, 0, 1, otherAccount.address, ethers.utils.formatBytes32String(""),
            );
        });
    ...
    describe("edit redemption", function () {
        it("should be success", async function () {
            const { realm, realmOp, otherAccount, erc20A, erc721A } = await loadFixture(deployRealmWithMultiTokensMark);
            await erc721A.safeMint(otherAccount.address);
            const mockCustomId = ethers.utils.formatBytes32String('mock-custom-id');
            await realm.connect(realmOp).editRedemption(erc721A.address, 0, mockCustomId, true);
            expect(await realm.isRedeemable(erc721A.address, 0, mockCustomId)).to.be.false;
            await realm.connect(realmOp).editRedemption(erc721A.address, 0, mockCustomId, false);
            expect(await realm.isRedeemable(erc721A.address, 0, mockCustomId)).to.be.true;
        });
        ...
        it("should be reverted when not OPERATOR role", async function () {
            ...
            const mockCustomId = ethers.utils.formatBytes32String('mock-custom-id');
            await realm.connect(realmOp).editRedemption(erc721A.address, 0, mockCustomId, true);
            await expect(realm.connect(otherAccount).editRedemption(erc721A.address, 0, mockCustomId, false))
                .to.be.revertedWith(`AccessControl: account ${otherAccount.address.toLowerCase()} is missing role ${ethers.utils.id("OPERATOR")}`);
        });
    });
